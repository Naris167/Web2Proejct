$(document).ready(function () {
      ...
      success: function (responseText) {
        ...
        try {
          ...
          if (data.success) {
            ...
            showNotification(
              "Failed to add item to cart: " + (data.error || "Unknown error"),
              "error"
            );
          }
        } catch (e) {
  ...
  $(document).on(
    "click",
    '.deleteFromCartForm button[type="submit"]',
    function (e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      success: function (responseText) {
        ...
        try {
          ...
          if (data.success) {
            ...
            showNotification(
              "Failed to removed item from cart: " +
  ...
  function updateCart() {
    $.ajax({
      ...
      error: function (xhr, status, error) {
        ...
        showNotification(
          "Failed to update cart. Please refresh the page.",
          "error"
        );
      },
    });
  ...
  function initQuantityControls() {
    $(".qty-up, .qty-down")
      ...
      .on("click", function (e) {
        e.preventDefault();
        var $button = $(this);
        var $input = $button.closest(".quantity-controls").find(".qty_input");
  ...
  function showNotification(message, type = "success") {
    ...
    var notification = $("<div></div>")
      .html(
        `<strong style="margin-right: 10px;">${icon}</strong><span>${message}</span>`
      )
      .css({
        position: "fixed",
        top: "-100px",
